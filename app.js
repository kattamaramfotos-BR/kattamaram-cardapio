...
const UI = {
  'pt-BR': {
    menu: '☰ MENU', lang: 'PT-BR', heroSmall: 'KATTAMARAM II • FOZ DO IGUAÇU', title: 'CARDÁPIO', aboard: 'a bordo', heroDesc: 'Sabores especiais para tornar seu passeio ainda melhor.', categories: 'NOSSAS CATEGORIAS', featured: 'DESTAQUES DO CARDÁPIO', products: 'PRODUTOS', options: 'OPÇÕES', optionAvailable: 'opção disponível', optionsAvailable: 'opções disponíveis', noProducts: 'Nenhum produto disponível nesta categoria.', footer: 'KATTAMARAM II • FOZ DO IGUAÇU • @KATTAMARAMFOZ'
  },
  en: {
    menu: '☰ MENU', lang: 'EN', heroSmall: 'KATTAMARAM II • FOZ DO IGUAÇU', title: 'MENU', aboard: 'on board', heroDesc: 'Special flavors to make your trip even better.', categories: 'OUR CATEGORIES', featured: 'MENU HIGHLIGHTS', products: 'PRODUCTS', options: 'OPTIONS', optionAvailable: 'option available', optionsAvailable: 'options available', noProducts: 'No products available in this category.', footer: 'KATTAMARAM II • FOZ DO IGUAÇU • @KATTAMARAMFOZ'
  },
  es: {
    menu: '☰ MENÚ', lang: 'ES', heroSmall: 'KATTAMARAM II • FOZ DO IGUAÇU', title: 'MENÚ', aboard: 'a bordo', heroDesc: 'Sabores especiales para hacer su paseo aún mejor.', categories: 'NUESTRAS CATEGORÍAS', featured: 'DESTACADOS DEL MENÚ', products: 'PRODUCTOS', options: 'OPCIONES', optionAvailable: 'opción disponible', optionsAvailable: 'opciones disponibles', noProducts: 'No hay productos disponibles en esta categoría.', footer: 'KATTAMARAM II • FOZ DO IGUAÇU • @KATTAMARAMFOZ'
  }
};

const LANG_NAMES = { 'pt-BR': 'PT-BR', en: 'ENGLISH', es: 'ESPAÑOL' };
...
function tr(obj, field) {
  const value = obj?.translations?.[currentLang]?.[field];
  return String(value ?? '').trim() || String(obj?.[field] ?? '');
}
function translationFor(obj, field) { return tr(obj, field); }
...
function updateStaticTexts() {
  ...
  $('#heroSmall').textContent = t('heroSmall');
  $('#heroTitle').childNodes[0].nodeValue = `${t('title')} `;
  $('#heroAboard').textContent = t('aboard');
  $('#heroDesc').textContent = t('heroDesc');
  $('#categoriesTitle').textContent = t('categories');
  $('#featuredTitle').textContent = t('featured');
  $('#title').textContent = t('products');
...
async function load() {
  ...
  if (c.error) { console.error(c.error); showError(currentLang === 'pt-BR' ? 'Não foi possível carregar o cardápio.' : currentLang === 'en' ? 'Could not load the menu.' : 'No fue posible cargar el menú.'); return; }

  const categories = c.data || [];
  const catsEl = $('#cats');
  catsEl.innerHTML = categories.map(x => {
    const name = translationFor(x, 'name');
    return `<button class="category" type="button" data-category-id="${x.id}" data-category-name="${esc(name)}">
      <div class="icon">${esc(x.icon || '•')}</div><strong>${esc(name)}</strong>
    </button>`;
  }).join('');
  ...
  if (categories.length) await loadCategory(categories[0].id, translationFor(categories[0], 'name'), false);
}
...
async function loadCategory(id, name, scroll = true) {
  ...
  if (r.error) { console.error(r.error); showError(currentLang === 'pt-BR' ? 'Não foi possível carregar os produtos.' : currentLang === 'en' ? 'Could not load the products.' : 'No fue posible cargar los productos.'); return; }
...
function render(el, items) {
  el.innerHTML = items.length ? items.map(p => {
    ...
    return `<article class="product" data-product-id="${p.id}">
      <img src="${esc(p.image_url || 'images/placeholder.svg')}" alt="${esc(name)}" loading="lazy">
      <div class="product-info">
        <div class="product-name">${esc(name)}</div>
        ${description ? `<div class="product-description">${esc(description)}</div>` : ''}
        <div class="price">${money(p.price)}</div>
        ${count ? `<div class="optionsHint">${count} ${count === 1 ? t('optionAvailable') : t('optionsAvailable')}</div>` : ''}
      </div>
    </article>`;
...
function openModal(p) {
  ...
  $('#modalOptions').innerHTML = opts.length ? `<div class="modalOptionsTitle">${t('options')}</div>` + opts.map(o => {
    const oname = tr(o, 'name');
    const odesc = tr(o, 'description');
    return `<div class="modalOption">
      ${o.image_url ? `<img src="${esc(o.image_url)}" alt="${esc(oname || '')}">` : ''}
      <div><strong>${esc(oname)}</strong>${odesc ? `<small>${esc(odesc)}</small>` : ''}</div>
...
function showError(message) {
  let el = $('#appError');
  if (!el) { el = document.createElement('div'); el.id = 'appError'; el.className = 'appError'; document.body.prepend(el); }
  el.textContent = message;
}

initLanguagePicker();
updateStaticTexts();
load().catch(err => { console.error(err); showError(currentLang === 'pt-BR' ? 'Erro ao carregar o cardápio.' : currentLang === 'en' ? 'Error loading the menu.' : 'Error al cargar el menú.'); });
